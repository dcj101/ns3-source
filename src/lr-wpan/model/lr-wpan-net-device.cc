...
LrWpanNetDevice::CompleteConfig (void)
{
  ...
  /**
SetMcpsDataIndicationCallback是ns3中的一个函数，用于设置接收数据包时的回调函数。
回调函数实现了IEEE 802.15.4的MCPS-DATA.indication SAP。
这个函数可以让你在数据包到达时执行一些自定义的操作，比如打印日志或统计信息。
如果你不想设置任何回调函数，你可以使用MakeNullCallback<>构造一个空的回调。  
*/
...
/**
用于构建一个“伪48位地址”，其形式是PanId:ShortAddr:00:00。
*/
