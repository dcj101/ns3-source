...
// 这里发送数据的时候开始构造MAC头部，里面传递的类型是DATA
  ...
  if (b0 == TX_OPTION_ACK)
    {
      NS_LOG_FUNCTION (this << "dcj come in ACK OP ----------------------");
      ...
      if (macHdr.GetDstAddrMode () == SHORT_ADDR)
        ...
        {
      NS_LOG_FUNCTION (this << "dcj come in SET ACK OP ----------------------");
...
/*
PdDataIndication 函数是由MAC层注册到 物理层处理完数据包之后 调用的PdDataIndication
用于将接收到的数据包传递给MAC层。它包含了数据包的内容、长度、信号强度等参数。2
*/
...
LrWpanMac::PdDataIndication (uint32_t psduLength, Ptr<Packet> p, uint8_t lqi)
{
  ...
  if (!receivedMacTrailer.CheckFcs (p))
  ...
  else
    {
      ...
      // 如果处于接收模式
      NS_LOG_DEBUG ("m_macPromiscuousMode is " << m_macPromiscuousMode);
      if (m_macPromiscuousMode)
      ...
      else
        {
          ...
          NS_LOG_DEBUG ("acceptFrame is " << acceptFrame);
          ...
          NS_LOG_DEBUG("1184 acceptFrame" << acceptFrame);
          ...
          NS_LOG_DEBUG("1206 acceptFrame" << acceptFrame);
          if (acceptFrame)
            {
              ...
              NS_LOG_DEBUG ("receivedMacHdr is " << receivedMacHdr.IsData () << " " << 
              ...
              if ((receivedMacHdr.IsData () || receivedMacHdr.IsCommand ()) && receivedMacHdr.IsAckReq ()
                  && !(receivedMacHdr.GetDstAddrMode () == SHORT_ADDR && (receivedMacHdr.GetShortDstAddr ().IsBroadcast () || receivedMacHdr.GetShortDstAddr ().IsMulticast ())))
                {
                  ...
                  // m_setMacState = Simulator::Schedule (Seconds(0.1),&LrWpanMac::SendAck, this, receivedMacHdr.GetSeqNum ());
                }
...
/*
PdDataConfirm 是一个函数，用于确认物理层数据传输的结束。
*/
...
LrWpanMac::PdDataConfirm (LrWpanPhyEnumeration status)
{
  ...
  if (status == IEEE_802_15_4_PHY_SUCCESS) // 如果
    {
      if (!macHdr.IsAcknowledgment ())
        {
          if (macHdr.IsBeacon ())
            ...
            {
              NS_LOG_FUNCTION (this << " ------>>>>>> i had receviced a ack packet");
...
Mac16Address 
LrWpanMac::GetAssociatedMac16AddressCoor()
{
...
Mac64Address 
LrWpanMac::GetAssociatedMac64AddressCoor()
{
